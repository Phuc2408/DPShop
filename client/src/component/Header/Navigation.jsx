...
export default function Navigation() {
  ...
  const getNavLinkClass = ({ isActive }) =>
    ...
    return (
        <div className="bg-[#f8f4e9] font-sans px-8 py-8 flex items-center justify-between rounded-lg shadow">
        ...
        {authLinks.map((link) => (
          <Link
            key={link.label}
            to={link.path}
            className="text-[#0A0A0A] hover:text-[#9F8A46] transition-colors"
          >
            {link.label}
          </Link>
        ))}
